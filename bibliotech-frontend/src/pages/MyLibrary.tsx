...
const MyLibrary = () => {
  ...
  const [userLibrary, setUserLibrary] = useState<UserLibraryDto[]>([]);
  const [purchasedBooks, setPurchasedBooks] = useState<BookResponse[]>([]); // Thêm state cho sách đã mua
  const [readingHistory, setReadingHistory] = useState<ReadingHistoryDto[]>([]);
  const [bookDetails, setBookDetails] = useState<Record<number, any>>({}); // Store book details by ID
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      setError(null);
      try {
        // Fetch user library, reading history, and purchased books in parallel
        const [libraryData, historyData, purchasedBooksData] = await Promise.all([
          usersService.getUserLibrary(),
          usersService.getReadingHistory(),
          transactionService.getMyPurchasedBooks(), // Lấy sách đã mua
        ]);

        setUserLibrary(libraryData);
        setReadingHistory(historyData);
        setPurchasedBooks(purchasedBooksData.data); // Lưu danh sách sách đã mua

        // Get unique book IDs from library, history, and purchased books
        const bookIds = new Set([
          ...libraryData.map(item => item.bookId),
          ...historyData.map(item => item.bookId),
          ...purchasedBooksData.data.map(book => book.bookId),
        ]);
  ...
  const fetchBookDetails = async (bookIds: number[]) => {
    try {
  ...
  const getBookCoverColor = (bookId: number) => {
  ...
  const shelves = [
    ...
    { id: "purchased", name: "Purchased Books" }, // Thêm shelf cho sách đã mua
    { id: "history", name: "Reading History" },
  ];

  // Kết hợp userLibrary với purchasedBooks
  const combinedLibrary = [
    ...userLibrary,
    ...purchasedBooks
      .filter(pb => !userLibrary.some(ul => ul.bookId === pb.bookId)) // Tránh trùng lặp
      .map(pb => ({
        bookId: pb.bookId,
        status: "PURCHASED", // Gán trạng thái mặc định cho sách đã mua
  ...
  const filteredBooks = combinedLibrary.filter(book => {
    ...
    if (selectedShelf === "purchased") return book.status === "PURCHASED"; // Lọc sách đã mua
    if (selectedShelf === "history") return false;
    return true;
  }).filter(book => {
  ...
  const historyBookIds =
    selectedShelf === "history"
      ? [...new Set(readingHistory.map(item => item.bookId))].filter(
          bookId => !combinedLibrary.some(book => book.bookId === bookId)
        )
      : [];
  ...
  const Bookshelf = ({ books, title }) => (
    <div className="relative mb-12">
      ...
      <div className={`relative rounded-lg ${isDarkMode ? "bg-gray-800/50" : "bg-white"} shadow-lg`}>
        <div className="relative grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6 p-8">
          {books.length > 0 ? (
                    ...
                    {bookInfo.coverImageUrl ? (
                      ...
                      >
                        <h3 className="text-sm font-bold mb-1 line-clamp-2">
                          {bookInfo.title || `Book #${book.bookId}`}
                        </h3>
            ...
            <div className="col-span-full py-8 text-center text-gray-500 dark:text-gray-400">
              {isLoading ? <p>Loading your books...</p> : <p>No books found in this shelf</p>}
            </div>
          )}
        </div>
        <div className={`absolute bottom-0 inset-x-0 h-4 ${isDarkMode ? "bg-gray-700" : "bg-amber-100"} rounded-b-lg`}>
          <div className="absolute inset-0 bg-gradient-to-r from-black/5 via-transparent to-black/5" />
        </div>
      </div>
      <div className="absolute -bottom-6 inset-x-8 flex justify-between">
        <div
          className={`w-1 h-6 ${isDarkMode ? "bg-gray-700" : "bg-amber-200"} rounded-b-lg shadow-md transform -skew-x-12`}
        />
        <div
          className={`w-1 h-6 ${isDarkMode ? "bg-gray-700" : "bg-amber-200"} rounded-b-lg shadow-md transform skew-x-12`}
        />
  ...
  const ReadingHistorySection = () => (
    <div className={`mt-8 p-6 rounded-lg ${isDarkMode ? "bg-gray-800/50" : "bg-white"} shadow-lg`}>
      ...
      {readingHistory.length > 0 ? (
              ...
              <div key={session.historyId || index} className="py-4 flex items-start gap-4">
                ...
                <div className="flex-1">
                  <h3 className="text-sm font-semibold mb-1">
                    {bookInfo.title || `Book #${session.bookId}`}
                  </h3>
        ...
        <div className="py-8 text-center text-gray-500 dark:text-gray-400">
          {isLoading ? <p>Loading your reading history...</p> : <p>No reading history found</p>}
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      ...
      >
        <header
          className={`w-full border-b ${isDarkMode ? "bg-gray-900 border-gray-700" : "bg-gray-50 border-gray-200"}`}
        >
      ...
      {selectedBook && (
        ...
        >
          <div className="p-6">
            ...
            <div className={`p-4 rounded-lg ${isDarkMode ? "bg-gray-700/50" : "bg-gray-50/50"}`}>
              ...
              <p className="text-sm mt-2 text-gray-500 dark:text-gray-400">
                Last read on{" "}
                {(selectedBook.lastReadDate && new Date(selectedBook.lastReadDate).toLocaleDateString()) || "Not yet"}
              </p>
              <div className="mt-4">
                <p className="text-sm font-medium">
                  Status: <span className="font-normal">{selectedBook.status}</span>
                </p>
              </div>
            </div>
            <div className="space-y-4 mt-6">
              <div className="flex gap-4">
                <motion.button
                  ...
                  onClick={() => {
                    usersService
                      .updateBookStatus(selectedBook.bookId, { status: "READING" })
                      .then(updatedBook => {
                        setUserLibrary(library =>
                          library.map(book => (book.bookId === updatedBook.bookId ? updatedBook : book))
                        );
                  ...
                  onClick={() => {
                    ...
                    const readingSession = {
                      ...
                      currentPage: Math.floor(
                        (selectedBook.progressPercentage / 100) * (selectedBook.bookInfo?.pageCount || 300)
                      ),
                    };
                    usersService
                      .recordReadingSession(readingSession)
                      .then(newSession => {
              ...
              <div className="flex gap-4">
                <motion.button
                  ...
                  onClick={() => {
                    usersService
                      .updateBookStatus(selectedBook.bookId, { status: "COMPLETED", progress: 100 })
                      .then(updatedBook => {
                        setUserLibrary(library =>
                          library.map(book => (book.bookId === updatedBook.bookId ? updatedBook : book))
                        );
                ...
                <motion.button
                  ...
                  onClick={() => {
                    usersService
                      .updateBookStatus(selectedBook.bookId, { status: "ABANDONED" })
                      .then(updatedBook => {
                        setUserLibrary(library =>
                          library.map(book => (book.bookId === updatedBook.bookId ? updatedBook : book))
                        );
